//Calculating memory usage and peak memory, as well as formatting its size
//Adapted from github.com/deanstef/awesome-plonky2
...
pub fn get_memory_usage() -> u64 {
    ...
    if let Some(pid) = sysinfo::get_current_pid().ok() {
        if let Some(process) = system.process(pid) {
            return process.memory() * 1024; // Convert from KB to bytes
        }
    }
